...
public class PlaceMonuments : ProceduralComponent
{
	...
	public override void Process(uint seed)
	{
		...
		else
		{
			...
			int num3 = Mathf.RoundToInt((float)TargetCount * TargetCountWorldSizeMultiplier.Evaluate(World.Size));
			for (int j = 0; j < 8; j++)
			{
				num = 0;
				a.Clear();
				...
				foreach (Prefab<MonumentInfo> prefab in array5)
				{
					...
					int num5 = 100000 * num4 * num4 * num4 * num4;
					int num6 = 0;
					int num7 = 0;
					...
					for (int k = 0; k < 10000; k++)
					{
						...
						if (num9 <= num7 || !prefab.ApplyTerrainFilters(pos, rot, scale) || !prefab.ApplyTerrainAnchors(ref pos, rot, scale, Filter) || !component.CheckPlacement(pos, rot, scale))
						{
							continue;
						}
						...
						if (!prefab.ApplyTerrainChecks(pos, rot, scale, Filter) || !prefab.ApplyWaterChecks(pos, rot, scale) || prefab.CheckEnvironmentVolumes(pos, rot, scale, EnvironmentType.Underground | EnvironmentType.TrainTunnels))
						{
							continue;
						}
						bool flag = false;
						...
						if (!flag)
						{
							...
							item = spawnInfo;
							num6++;
							if (num6 >= 8 || DistanceDifferentType == DistanceMode.Any)
							{
								break;
							}
						}
