...
internal class Trans_TargetOrSelfInWater : FSMTransitionBase
{
	protected override bool EvaluateInternal()
	{
		...
		{
			if (base.Senses.FindTarget(out var target) && target.ToNonNpcPlayer(out var _) && !NavHelpers.IsAcceptableWaterDepth(Owner, target.transform.position, Owner.bounds.extents.y))
			{
				return true;
			}
