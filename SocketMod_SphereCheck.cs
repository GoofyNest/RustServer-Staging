	...
	public override bool DoCheck(Construction.Placement place)
	{
		...
		if (requireMonument)
		{
			for (int i = 0; i < obj.Count; i++)
			{
				...
				if (!collider.gameObject.HasCustomTag(GameObjectTag.BlockBarricadePlacement) && (collider.GetMonument() == null || collider.gameObject.HasCustomTag(GameObjectTag.AllowBarricadePlacement)))
				{
					obj.RemoveAt(i);
					i--;
				}
