...
public class Admin : ConsoleSystem
{
	...
	public static void status(Arg arg)
	{
		...
		if (!flag && @string.Length == 0)
		{
			...
			text = text + "version : " + 2569 + " secure (secure mode enabled, connected to Steam3)\n";
	...
	public static void clear_wallpaper_radius(Arg arg)
	{
		...
		RunInRadius(arg.GetFloat(0), arg.Player(), delegate(BuildingBlock block)
		{
	...
	{
		BuildingBlock buildingBlock = GamePhysics.RaycastEntity(GamePhysics.Realm.Server, new Ray(position, direction), maxDistance, 10485760, QueryTriggerInteraction.Ignore) as BuildingBlock;
	...
	{
		IEnumerable<BuildingBlock> blocks = SearchRadius(arg.Player().transform.position, arg.GetFloat(1));
	...
	private static void wallpaper_radius_internal(Arg arg, bool addIfMissing)
	{
		...
		for (int i = 0; i < skins.Length; i++)
		{
			...
			if (skinIdParsed == -1 && (skin.invItem.displayName.english.Contains(@string, StringComparison.InvariantCultureIgnoreCase) || skin.invItem.name.Contains(@string, StringComparison.InvariantCultureIgnoreCase)))
			{
				flag = true;
	...
	public static void clientperf_frametime(Arg arg)
	{
		...
		foreach (BasePlayer activePlayer in BasePlayer.activePlayerList)
		{
			activePlayer.ClientRPC(RpcTarget.Player("GetPerformanceReport_Frametime", activePlayer), JsonConvert.SerializeObject(value));
		}
	}
	...
	public static void entid(Arg arg)
	{
		...
		if (arg.Player() != null)
		{
			Debug.Log("[ENTCMD] " + arg.Player().displayName + "/" + arg.Player().userID.Get() + " used *" + @string + "* on ent: " + baseEntity.name);
		}
		switch (@string)
		{
		case "kill":
	...
	private static bool RunInRadius<T>(float radius, BaseEntity initial, Action<T> callback, Func<T, bool> filter = null) where T : BaseEntity
	{
		...
		if (radius > 0f)
		{
			global::Vis.Entities(initial.transform.position, radius, obj, 2097152);
		}
	...
	public static void AdminUI_RequestUGCContent(Arg arg)
	{
		if (allowAdminUI && !(arg.Player() == null))
		{
			...
			if (array != null)
			{
				...
				arg.Player().ClientRPC(RpcTarget.SendInfo("AdminReceivedUGC", sendInfo2), uInt, (uint)array.Length, array, uInt2, (byte)@int);
			}
		}
	}
	...
	public static void AdminUI_RequestFireworkPattern(Arg arg)
	{
		if (allowAdminUI)
		{
			...
			if (baseNetworkable != null && baseNetworkable is PatternFirework patternFirework)
			{
				...
				sendInfo.method = SendMethod.Reliable;
				SendInfo sendInfo2 = sendInfo;
				arg.Player().ClientRPC(RpcTarget.SendInfo("AdminReceivedPatternFirework", sendInfo2), entityID, patternFirework.Design.ToProtoBytes());
			}
		}
	}
