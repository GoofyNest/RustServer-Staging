...
public class Server : ConsoleSystem
{
	...
	public static float GetTaxRateForWorkbenchUnlock(int workbenchLevel)
	{
		...
		{
		case 0:
			value = workbench1TaxRate;
			break;
		case 1:
			value = workbench2TaxRate;
			break;
		case 2:
			value = workbench3TaxRate;
			break;
		}
	...
	public static void playerlistpos(Arg arg)
	{
		...
		foreach (BasePlayer activePlayer in BasePlayer.activePlayerList)
		{
			textTable.AddRow(activePlayer.userID.Get().ToString(), activePlayer.displayName, activePlayer.transform.position.ToString(), activePlayer.eyes.BodyForward().ToString());
		}
