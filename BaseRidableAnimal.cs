...
public class BaseRidableAnimal : BaseVehicle
{
	...
	{
		using (TimeWarning.New("BaseRidableAnimal.VehicleFixedUpdate"))
		{
	...
	public virtual void RiderInput(InputState inputState, BasePlayer player)
	{
		...
		{
			float num = ((currentRunState > RunState.walk) ? 1f : 0.5f);
			if (currentRunState != 0 && timeInMoveState > num)
			{
				ModifyRunState(-1);
			}
		}
	...
	public float ObstacleDistanceCheck(float speed = 10f)
	{
		...
		float num4 = 0f;
		Vector3 vector = ((currentRunState == RunState.reverse) ? (-base.transform.forward) : base.transform.forward);
		Vector3 vector2 = ((currentRunState == RunState.reverse) ? Vector3.back : Vector3.forward);
		Vector3 vector3 = QuaternionEx.LookRotationForcedUp(vector, Vector3.up) * vector2;
		Vector3 vector4 = ((currentRunState == RunState.reverse) ? animalRear.transform.position : movementLOSOrigin.transform.position);
		...
		for (int i = 0; i < num3; i++)
		{
			...
			Vector3 origin = vector5 + Vector3.up * (maxStepHeight + obstacleDetectionRadius);
			Vector3 vector6 = vector5 + vector3 * num5;
			float num7 = maxStepDownHeight + obstacleDetectionRadius;
			if (GamePhysics.Trace(new Ray(origin, vector3), obstacleDetectionRadius, out var hitInfo, num5, 1486954753))
			{
			...
			if (!flag)
			{
				if (!TransformUtil.GetGroundInfo(vector6 + Vector3.up * 2f, out pos, out normal, 2f + num7, 429981697))
				{
					return num4;
				}
				num6 = Vector3.Distance(vector5, pos);
			...
			if (flag)
			{
				...
				{
					Vector3 vector7 = normal;
					float num10 = pos.y;
					int num11 = 1;
					for (int j = 0; j < normalOffsets.Length; j++)
					{
						Vector3 vector8 = vector6 + normalOffsets[j].x * base.transform.right;
						float num12 = maxStepHeight * 2.5f;
						if (TransformUtil.GetGroundInfo(vector8 + Vector3.up * num12 + normalOffsets[j].z * vector, out var pos2, out var normal2, num7 + num12, 429981697))
						{
							num11++;
							vector7 += normal2;
							num10 += pos2.y;
					...
					num10 /= (float)num11;
					vector7.Normalize();
					float num13 = Vector3.Angle(up, vector7);
					num9 = Vector3.Angle(vector7, Vector3.up);
					if (num13 > maxWallClimbSlope || num9 > maxWallClimbSlope || Mathf.Abs(num10 - vector6.y) > maxStepHeight)
					{
						return num4;
					}
				}
			...
			num4 += num6;
			vector3 = QuaternionEx.LookRotationForcedUp(vector, normal) * vector2;
			vector4 = pos;
		}
		return num4;
	}
	...
	public void UpdateMovement(float delta)
	{
		...
		if (desiredRotation != 0f)
		{
			...
			bool flag2 = GamePhysics.OverlapSphere(animalHead.transform.position, obstacleHeadDetectionRadius, 2162688);
			bool num12 = GamePhysics.OverlapSphere(animalNeck.transform.position, obstacleDetectionRadius * 1.8f, 2097152);
			bool flag3 = GamePhysics.OverlapSphere(animalRear.transform.position, obstacleDetectionRadius, 65536);
			bool flag4 = GamePhysics.OverlapSphere(animalRear.transform.position, obstacleDetectionRadius * 1.8f, 2097152);
			if ((num12 || flag2 || flag3 || flag4) && !IsLeading())
			{
				base.transform.rotation = rotation;
				base.transform.position = position2;
			}
			if (Vis.AnyColliders(animalNeck.transform.position, obstacleHeadDetectionRadius * 0.25f, 1503731969) || Vis.AnyColliders(animalFront.transform.position, obstacleHeadDetectionRadius * 0.25f, 1503731969))
			{
				base.transform.rotation = rotation;
			}
		}
		...
		Vector3 vector2 = base.transform.position + normalized * num13 * Mathf.Sign(currentSpeed);
		...
		Transform transform = ((currentRunState > RunState.reverse) ? animalFront : animalRear);
		Vector3 origin = base.transform.position + base.transform.InverseTransformPoint(transform.position).y * base.transform.up;
		bool flag5 = false;
		bool flag6 = false;
		bool flag7 = false;
		bool flag8 = false;
		bool flag9 = false;
		bool flag10 = GamePhysics.CheckBounds(WorldSpaceBounds().ToBounds(), 2097152);
		RaycastHit hitInfo;
		if (currentRunState == RunState.reverse)
		{
			flag9 = GamePhysics.OverlapSphere(animalRear.transform.position, obstacleDetectionRadius, 2162688);
		}
		else
		{
			flag5 = GamePhysics.Trace(new Ray(animalFront.transform.position, normalized), obstacleHeadDetectionRadius, out hitInfo, num13, 1503731969);
			flag7 = GamePhysics.OverlapSphere(animalHead.transform.position, obstacleHeadDetectionRadius, 2162688);
		}
		bool flag11 = false;
		if (AnyMounted())
		{
			foreach (MountPointInfo allMountPoint in base.allMountPoints)
			{
				...
				if (!(mounted == null))
				{
					flag11 = GamePhysics.OverlapSphere(mounted.eyes.position, obstacleHeadDetectionRadius, 65536);
					if (flag11)
					{
						break;
					}
		...
		flag6 = GamePhysics.Trace(new Ray(origin, normalized), obstacleDetectionRadius, out hitInfo, num13, 1503731969);
		flag8 = GamePhysics.OverlapSphere(animalNeck.transform.position, obstacleDetectionRadius * 2.3f, 2097152);
		bool flag12 = Vis.AnyColliders(transform.transform.position + normalized * num13, obstacleHeadDetectionRadius, 1503731969);
	...
	public bool DropToGround(Vector3 targetPos, bool force = false)
	{
		...
		{
			if (GamePhysics.CheckSphere(pos + Vector3.up * 1f, 0.2f, 429981697))
			{
				return false;
			}
