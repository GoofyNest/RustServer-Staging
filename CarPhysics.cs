	...
	public void FixedUpdate(float dt, float speed)
	{
		...
		if (!rBody.IsSleeping())
		{
			...
			else
			{
				...
				int num4 = 0;
				float num5 = 0f;
				bool flag2 = !hasDriver && rBody.velocity.magnitude < 2.5f && (float)car.timeSinceLastPush > 2f && car.OnSurface != VehicleTerrainHandler.Surface.Frictionless;
				bool flag3 = !flag2 && num2 == 0f && num < 0.2f && (float)car.timeSinceLastPush > 2f && car.OnSurface != VehicleTerrainHandler.Surface.Frictionless;
